...
function Profil() {
    ...
    const postUserId = UserParams.userId;
    ...
    useEffect(() => {
        async function makeRequest() {      
            await delay(2000);
        axios.get(`http://localhost:3000/api/posts/${postUserId}`, {
     ...
     posts[0] ? profilTitle  = posts[0].pseudo : profilTitle = userPseudo + ", vous n'avez encore rien posté";
    ...
    return isLoading ? ( 
        ...
        <div>
            <PostsTitle> {profilTitle} </PostsTitle>
            <div>
                <PostsContainer>
                    {posts.map((post)=> (
